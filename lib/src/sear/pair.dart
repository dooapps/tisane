...
Future<PairReturnType> pair() async {
  final signKeys = await crypto.EcdsaPrivateKey.generateKey(
  ...
  final sa = {'priv': signPri['d'], 'pub': "${signPub["x"]}.${signPub["y"]}"};

  final cryptKeys = await crypto.EcdhPrivateKey.generateKey(
    crypto.EllipticCurve.p256,
  );
